(* OASIS_START *)
(* DO NOT EDIT (digest: e96bf35cce1b6629df738adf1554a306) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("fold", ["src"], [])];
     lib_c = [];
     flags =
       [
          (["oasis_executable_test_parser_byte"; "ocaml"; "link"; "byte"],
            [(OASISExpr.EBool true, S [A "-g"])]);
          (["oasis_executable_test_parser_byte"; "ocaml"; "ocamldep"; "byte"],
            [(OASISExpr.EBool true, S [A "-g"])]);
          (["oasis_executable_test_parser_byte"; "ocaml"; "compile"; "byte"],
            [(OASISExpr.EBool true, S [A "-g"])])
       ];
     includes = [("tests", ["src"])]
  }
