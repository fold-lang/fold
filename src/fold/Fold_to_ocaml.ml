...
module Ml_eval = struct
  ...
  type str = Parsetree.structure_item
  ...
  module Exp = struct
    ...
    let apply eval f_fl args_fl =
      let f_ml = eval.exp f_fl in
      let args_ml =
        List.map (fun arg_fl -> (Asttypes.Nolabel, eval.exp arg_fl)) args_fl
      in
    ...
    let tuple eval items = Ml.Exp.tuple (List.map eval.exp items)

    let rec list eval ?spread:tl xs =
      match xs with
      | [] -> Option.fold ~some:eval.exp ~none:nil tl
      | x_fl :: xs_fl ->
        let x_ml = eval.exp x_fl in
    ...
    let rec block eval (xs : fl list) =
      ...
      | item :: items -> Ml.Exp.sequence (eval.exp item) (block eval items)

    let if_then_else eval cond_fl then_fl else_fl =
      let cond_ml = eval.exp cond_fl in
      let then_ml = eval.exp then_fl in
      let else_ml = eval.exp else_fl in
    ...
    let while' eval cond body =
      let cond_ml = eval.exp cond in
      let body_ml = eval.exp body in
    ...
    and if_cases eval (cs0 : fl list) =
      let rec loop (cs : fl list) =
        ...
        | Shape ("->", [ Ident (Lower "else"); else_body ]) :: _cs ->
          let else_body = eval.exp else_body in
          Some else_body
        | Shape ("->", [ cond; body ]) :: cs ->
          let cond = eval.exp cond in
          let body = eval.exp body in
  ...
  module Mod = struct
    let structure eval items_fl =
      let items_ml = List.map eval.str items_fl in
  ...
  module Str = struct
    ...
    let eval eval fl = Ml.Str.eval (eval.exp fl)
