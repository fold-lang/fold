...
open Migrate_parsetree.Ast_500
...
structure_item:
  ...
  | wrap_mkstr_ext(
      primitive_declaration
        { pstr_primitive $1 }
    (* --- BEGIN FOLD --- *)
    (* Value declaration is no longer permitted.
       The current usage seems to only produce an error anyway. *)
    (*| value_description
        { pstr_primitive $1 }*)
    (* --- END FOLD --- *)
    | type_declarations
...
val_bindings(EXT):
    val_binding(EXT)                            { $1 }
  | val_bindings(EXT) and_let_binding           { addlb $1 $2 }
;
%inline val_binding(EXT):
  VAL
  ext = EXT
  attrs1 = attributes
  rec_flag = rec_flag
