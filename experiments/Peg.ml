...
let rec parse self : 'a list P.t =
  ...
  | Epsilon -> P.pure []
  | Terminal x ->
    P.expect (Symbol x) >>= fun tok -> P.advance >> lazy (P.pure [ tok ])
  | Non_terminal x -> fail "not implemented"
  | Alternative [] -> P.pure []
  | Alternative [ x ] -> parse x
  | Alternative [ x; y ] -> parse x <|> parse y
  | Alternative (x :: xs) ->
  ...
  | Optional x -> parse x <|> P.pure []
  | Many peg -> P.many (parse peg) >>= fun xss -> P.pure (List.concat xss)
  | Some peg -> P.some (parse peg) >>= fun xss -> P.pure (List.concat xss)
