    ...
    let rec loop t =
      let desc =
        ...
        | Ptyp_constr ({ txt = Longident.Lident s }, [])
          when List.mem s var_names -> Ptyp_var s
...
(** Type extensions *)
module Te = struct
  let mk ?(loc = !default_loc) ?(attrs = []) ?(docs = empty_docs) ?(params = [])
      ?(priv = Public) path constructors =
    { ptyext_path = path
...
module Rf = struct
  ...
  let tag ?loc ?attrs label const tys = mk ?loc ?attrs (Rtag (label, const, tys))
